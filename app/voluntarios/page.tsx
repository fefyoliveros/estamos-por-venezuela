...
function SkillContactButton({ method, value }: { method?: string; value?: string }) {
  if (!value) return null
  const isUrl = value.startsWith('http')
  const isPhone = !isUrl && (value.startsWith('+') || /^\d/.test(value))
  if (method === 'whatsapp' && isPhone) {
    return (
      <a
            ...
            {offers.map((offer) => {
              ...
              return (
                <div key={offer.id} className="card">
                  ...
                  <SkillContactButton method={offer.contact_method} value={offer.contact_value} />
                </div>
              )
            })}
          </div>
