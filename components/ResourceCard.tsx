    ...
    <article className="card flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`badge ${TYPE_COLORS[resource.type] ?? 'bg-brand-warm-surface text-brand-warm-muted'}`}>
      ...
      <div>
        <h3 className="font-semibold text-brand-ink text-base leading-snug">{resource.name}</h3>
        {resource.city && (
          <p className="text-xs text-brand-warm-muted mt-0.5">{resource.city}</p>
        )}
      </div>

      {description && (
        <p className="text-sm text-brand-ink/60 leading-relaxed line-clamp-3">{description}</p>
      )}

      {resource.contact && (
        <p className="text-xs text-brand-warm-muted bg-brand-warm-surface rounded-xl px-3 py-2">{resource.contact}</p>
      )}
      ...
      <div className="flex items-center gap-2 mt-auto pt-1 flex-wrap">
        {resource.url && (
          <a
            ...
            className="text-xs font-semibold text-brand-ember hover:text-brand-ember/80 underline underline-offset-2"
          >
            {t('directory.visit')} →
          </a>
        )}
